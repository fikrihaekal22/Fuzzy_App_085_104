...
//step 2 : implikasi
    ...
    function inferensi($suhu, $kelembapan, $tinggiair) //bagian implikasi komposisi
{
    echo "<h4><b>Rules yang digunakan: </b></h4>";
    $x = 0; //nilai array ke n
    $no = 1; //ini buat nilai rule yang keluar
    $kondisi = [];//nyimpen si irigasi (output) nilai output.
    ...
    for ($i = 0; $i < 3; $i++) {
        for ($j = 0; $j < 3; $j++) {
            for ($k = 0; $k < 3; $k++) {
                if (($nilaisuhu[$i] > 0) && ($nilaikelembapan[$j] > 0) && ($nilaitinggiair[$k] > 0)) { //nilai fuzzifikasi yang "0"
                    $minimal[$x] = min($nilaisuhu[$i], $nilaikelembapan[$j], $nilaitinggiair[$k]);
                    if ($k == 0) { //nilai dari tinggi air
                        $kondisi[$x] = "Banyak";
                    } else if (($i == 2) && ($k == 1) && ($j < 2)) { //i=2 itu dari suhu maksimal ( dalam hal ini adlaah kondisi suhu )
                        $kondisi[$x] = "Banyak";
                    } else {
                        $kondisi[$x] = "Sedikit";
                    }
    ...
    $nilai_banyak = 0;
    $nilai_sedikit = 0;//
    for ($l = 0; $l < $x; $l++) { //$l itu adalah mengambil iterasi perulangan sampai banyaknya rule yang di pakai
        if ($kondisi[$l]  == "Banyak") {
            $nilai_banyak = max($minimal[$l], $nilai_banyak); //nilai minimal L > nilai banyak ? then di simpan di $nilai banyak
        } else {
